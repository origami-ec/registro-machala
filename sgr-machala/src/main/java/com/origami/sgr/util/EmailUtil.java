...
public class EmailUtil implements Serializable {
    ...
    public Boolean sendAuthEmail(String to, String subject, String content, String bcc, String cc) throws UnsupportedEncodingException {
        boolean auth = true;
        try {
            final EmailConfigs ec = this.getConfigs();           
            Integer port = ec.getMailServerPort();
            ...
            Message message = new MimeMessage(sess);
            message.setFrom(new InternetAddress(ec.getMailServerUsername(), "REGISTRO DE LA PROPIEDAD DE PORTOVIEJO"));
            message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(to));
            if (bcc != null) {
                message.addRecipients(Message.RecipientType.BCC,
                        InternetAddress.parse(bcc));
