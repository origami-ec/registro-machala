...
public class BpmBaseEngineEJb implements BpmBaseEngine {
    ...
    public Integer getProcessDefinitionVersion(String processId) {
        return getProcessEngine().getRepositoryService().createProcessDefinitionQuery().processDefinitionId(processId).singleResult().getVersion();
    }
...
//            this.getProcessEngine().close();
    ...
    public ProcessInstance reactivateFromLastTask(String processInstanceId) {
        try {
            ...
            // 1. Obtener información del proceso histórico
            HistoricProcessInstance historicInstance = getProcessEngine().getHistoryService()
            ...
            if (historicInstance.getEndTime() == null) {
                throw new RuntimeException("El proceso aún está activo, no se puede reactivar");
            }

            // 3. Obtener TODAS las variables del histórico
            Map<String, Object> allVariables = new HashMap<>();

            List<HistoricVariableInstance> variableInstances = getProcessEngine().getHistoryService()
                    .createHistoricVariableInstanceQuery()
                    .processInstanceId(processInstanceId)
                    .list();

            for (HistoricVariableInstance varInstance : variableInstances) {
                allVariables.put(varInstance.getVariableName(), varInstance.getValue());
            }

            // 3. Encontrar la última tarea completada
            HistoricTaskInstance lastTask = getProcessEngine().getHistoryService()
                    ...
                    .findFirst()
                    .orElseThrow(() -> new RuntimeException("No se encontraron tareas completadas"));

            // 5. Crear nueva instancia CON las variables
            ProcessInstance newInstance;
            if (historicInstance.getBusinessKey() != null) {
                newInstance = getProcessEngine().getRuntimeService().startProcessInstanceByKey(
            ...
            this.moveToSpecificTask(processInstanceId, processInstanceId);
    ...
    private void moveToSpecificTask(String newProcessInstanceId, String taskDefinitionKey) {
        try {
            // Opción 1: Usar señal para activar el flujo hasta esa tarea
            List<Execution> executions = getProcessEngine().getRuntimeService().createExecutionQuery()
                    .processInstanceId(newProcessInstanceId)
                    .list();

            // Encontrar la ejecución adecuada y moverla
            for (Execution execution : executions) {
                getProcessEngine().getRuntimeService().trigger(execution.getId());
            }

            // Opción 2: Completar tareas automáticamente hasta llegar a la deseada
            this.moveThroughProcess(newProcessInstanceId, taskDefinitionKey);

        } catch (Exception e) {
            throw new RuntimeException("Error moviendo a la tarea específica: " + e.getMessage(), e);
        }
    }

    private void moveThroughProcess(String processInstanceId, String targetTaskDefinitionKey) {

        List<Task> currentTasks = getProcessEngine().getTaskService().createTaskQuery()
                .processInstanceId(processInstanceId)
                .list();

        while (!currentTasks.isEmpty()) {
            for (Task task : currentTasks) {
                // Si encontramos la tarea objetivo, la dejamos activa
                if (task.getTaskDefinitionKey().equals(targetTaskDefinitionKey)) {
                    System.out.println("Tarea objetivo encontrada: " + task.getName());
                    return;
                }

                // Completar tareas automáticas (service tasks, etc.)
                if (isAutomaticTask(task)) {
                    getProcessEngine().getTaskService().complete(task.getId());
                } // Para user tasks, podrías asignarlas automáticamente
                else if (isUserTask(task)) {
                    // Asignar a alguien o auto-completar con variables
                    Map<String, Object> variables = new HashMap<>();
                    // Puedes agregar variables por defecto
                    getProcessEngine().getTaskService().complete(task.getId(), variables);
                }
            }

            // Obtener siguiente conjunto de tareas
            currentTasks = getProcessEngine().getTaskService().createTaskQuery()
                    .processInstanceId(processInstanceId)
                    .list();
        }
    }

    private void restoreProcessVariables(String oldProcessInstanceId, String newProcessInstanceId) {

        Map<String, Object> historicVariables = getProcessEngine().getRuntimeService().getVariables(oldProcessInstanceId);

        // También obtener variables históricas específicas
        List<HistoricVariableInstance> historicVariableInstances = getProcessEngine().getHistoryService()
                .createHistoricVariableInstanceQuery()
                .processInstanceId(oldProcessInstanceId)
                .list();

        Map<String, Object> variablesToSet = new HashMap<>();
        for (HistoricVariableInstance historicVar : historicVariableInstances) {
            variablesToSet.put(historicVar.getVariableName(), historicVar.getValue());
        }

        // Establecer variables en la nueva instancia
        getProcessEngine().getRuntimeService().setVariables(newProcessInstanceId, variablesToSet);
    }

    private void restoreTaskVariables(String historicTaskId, String newProcessInstanceId) {

        List<HistoricVariableInstance> taskVariables = getProcessEngine().getHistoryService()
                .createHistoricVariableInstanceQuery()
        ...
        if (!taskVariables.isEmpty()) {
            ...
            getProcessEngine().getRuntimeService().setVariables(newProcessInstanceId, variables);
        }
    }

    public List<Task> getAvailableTasksAfterReactivation(String newProcessInstanceId) {
        return getProcessEngine().getTaskService().createTaskQuery()
                .processInstanceId(newProcessInstanceId)
                .list();
    }

    private boolean isAutomaticTask(Task task) {
        // Lógica para identificar tareas automáticas
        // Depende de cómo hayas definido tus tareas en el BPMN
        return task.getName() != null
                && (task.getName().toLowerCase().contains("service")
                || task.getName().toLowerCase().contains("automatic"));
    }

    private boolean isUserTask(Task task) {
        // Asumimos que es user task si no es automática
        // En la práctica, deberías verificar el tipo desde el BPMN
        return !isAutomaticTask(task);
    }
