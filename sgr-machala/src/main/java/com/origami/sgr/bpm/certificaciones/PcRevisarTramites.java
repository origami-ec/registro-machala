...
@Named
@ViewScoped
public class PcRevisarTramites extends BpmManageBeanBaseRoot implements Serializable {
    ...
    public void updateLiquidacion() {
        try {
            ...
            JsfUti.redirectFaces("/procesos/certificacion/revisarTramite.xhtml");
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
    ...
    public void showDlgComentarioProceso(Boolean aprobado) {
        try {
            this.solicitudAprobada = aprobado;
            JsfUti.update("formObs");
            JsfUti.executeJS("PF('dlgObsvs').show();");
        } catch (Exception e) {
            JsfUti.messageError(null, Messages.error, "");
    ...
    public void completarTareaControl() {
        try {
            ...
            if (solicitudAprobada) {
                ...
                par.put("aprobado", 3);
                par.put("directorArea", itl.getUsuarioByRolName("director_certificacion_archivo"));
            }
            this.reasignarTarea(this.getTaskId(), session.getName_user());
            this.completeTask(this.getTaskId(), par);
            this.continuar();
    ...
    public void completarTareaDirector() {
        try {
            this.guardarObservacion();
            par = new HashMap<>();
            if (solicitudAprobada) {
                ...
                par.put("aprobado", 2);
                par.put("asesorJuridico", itl.getUsuarioByRolName("director_asesoria_juridica"));
            }
            this.reasignarTarea(this.getTaskId(), session.getName_user());
            this.completeTask(this.getTaskId(), par);
            this.continuar();
