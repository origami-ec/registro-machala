...
@Named
@ViewScoped
public class InscribirPI extends BpmManageBeanBaseRoot implements Serializable {
    ...
    public void realizarTarea() {
        try {
            ...
            if (this.camposObligatorioInscripcion()) {
                ...
                tt = reg.guardarRegpTareasTramite(tt);
                JsfUti.redirectFaces("/procesos/inscripciones/InscripcionDocumento.xhtml");
            }
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
    ...
    public void regresarControlCalidad() {
        ...
        ss.agregarParametro("taskID", this.getTaskId());
        ss.agregarParametro("tramite", this.getTaskId());
        JsfUti.redirectFaces("/procesos/inscripciones/controlCalidad.xhtml");
    }
 ...
 /*public void ejecutarServicio() {
    ...
    public void llenarInfo(RegMovimiento m) {
        try {
            ...
            RegMovimientoReferencia mr;
            List<RegMovimientoReferencia> list;
    ...
    public void crearSecuenciaContrato() {
        try {
            RegpTareasTramite nuevaTarea = new RegpTareasTramite();
            nuevaTarea.setTramite(movimiento.getTramite().getTramite());
            nuevaTarea.setDetalle(movimiento.getTramite().getDetalle());
            nuevaTarea.setFecha(new Date());
            manager.persist(nuevaTarea);
            this.regresarCompletarTarea();
        } catch (Exception e) {
            JsfUti.messageError(null, Messages.error, "");
            LOG.log(Level.SEVERE, null, e);
