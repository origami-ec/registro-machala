...
public class InscripcionEdicion extends BpmManageBeanBaseRoot implements Serializable {
    ...
    public void showDlgNewInterv() {
        try {
            ...
            re.setNacionalidad(new UafNacionalidad(65L));
    ...
    public void showDlgEditInterv(int index) {
        try {
            ...
            if (interv.getEnteInterv().getNacionalidad() == null) {
                interv.getEnteInterv().setNacionalidad(new UafNacionalidad(65L));
            }
    ...
    public void agregarListaIntervEdit() {
        ...
        if (interv.getEnteInterv().getEstadoCivil() != null) {
            interv.setEstado(interv.getEnteInterv().getEstadoCivil().getValor());
        }
    ...
    public List<UafNacionalidad> getNacionalidades() {
        return em.findAllEntCopy(Querys.getUafNacionalidades);
    }

    public void redirectEdicionFicha() {
        try {
            if (ficha != null && ficha.getId() != null) {
                ss.instanciarParametros();
                ss.agregarParametro("idFicha", ficha.getId());
    ...
    /*public List<CtlgItem> getCargosRep() {
        map = new HashMap<>();
        map.put("catalogo", Variables.cargosRepresenante);
        return manager.findNamedQuery(Querys.getCtlgItemListByNombreDeCatalogo, map);
    }*/
