...
@Named
@ViewScoped
public class SupervisarTramite extends BpmManageBeanBaseRoot implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(SupervisarTramite.class.getName());
    ...
    public void generarPronunciamiento(Long idNotaDevolutiva) {
        ss.borrarDatos();
        ss.instanciarParametros();
        ss.setTieneDatasource(true);
    ...
    public void completarTareaReasignar() {
        try {
            if (us != null && us.getId() != null) {
                ...
                par = new HashMap<>();
                par.put("aprobado", 1);
                par.put("legal", us.getUsuario());
                par.put("inscriptor", us.getUsuario());
                par.put("certificador", us.getUsuario());
                this.guardarObservacion();
                this.reasignarTarea(this.getTaskId(), session.getName_user());
                this.completeTask(this.getTaskId(), par);
                liquidacion.setInscriptor(us);
                manager.merge(liquidacion);
                this.continuar();
            } else {
