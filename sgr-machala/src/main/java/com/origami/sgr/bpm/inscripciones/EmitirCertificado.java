...
@Named
@ViewScoped
public class EmitirCertificado extends BpmManageBeanBaseRoot implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(EmitirCertificado.class.getName());
    ...
    public void seleccionarCertificado() {
        try {
            ...
            ss.agregarParametro("taskID", this.getTaskId());
            ss.agregarParametro("formulario", "/procesos/inscripciones/emitirCertificado.xhtml");
            session.setTaskID(this.getTaskId());
            JsfUti.redirectFaces("/procesos/registro/certificar.xhtml");
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, null, e);
