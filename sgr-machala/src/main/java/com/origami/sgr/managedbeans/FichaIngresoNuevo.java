...
public class FichaIngresoNuevo extends BpmManageBeanBaseRoot implements Serializable {
    ...
    protected List<RegFichaLinderos> linderantes = new ArrayList<>();

    protected RegFichaLinderos lindero = new RegFichaLinderos();
    ...
    protected Boolean administrador = false, analistaSenior = false, jefeCertificacion = false;
    ...
    public void validaRoles() {
        administrador = session.getRoles().contains(1L);         //rol administrador
        jefeCertificacion = session.getRoles().contains(59L);     //rol jefe certificacion
        analistaSenior = session.getRoles().contains(96L);        //rol analsita senior
    }
    ...
    public void editarMovimiento(RegMovimiento m) {
        if (m.getEditable()) {
            ...
            if (ficha != null && ficha.getId() != null) {
                ...
                ficha = reg.guardarFichaRegistral(ficha, propietarios, movimientos, linderantes);
 ...
 /*public void findTramite() {
    ...
    public void saveFicha() {
        try {
            if (this.validaCampos()) {
                ficha.setDireccionBien(ficha.getUafTipoBien().getDescripcion());
                if (ficha.getId() == null) {
    ...
    public boolean validaCampos() {
        ...
        if (ficha.getParroquia() == null) {
            JsfUti.messageWarning(null, Messages.faltanCampos, "Falta seleccionar la parroquia de la ficha.");
            return false;
        }
        ...
        if (ficha.getDescripcionBien() == null || ficha.getDescripcionBien().isEmpty()) {
            JsfUti.messageWarning(null, Messages.faltanCampos, "Debe ingresar la descripción/ubicación del bien inmueble.");
            return false;
        }
        /*if (linderantes.isEmpty()) {
            JsfUti.messageWarning(null, Messages.faltanCampos, "Debe ingresar los linderos del bien inmueble.");
            return false;
        }*/
        return true;
    }
    ...
    public void cargarListasPersistidas() {
        try {
            ...
            linderantes = reg.getLinderantesFichaByFicha(ficha.getId());
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
    }
    ...
    public void showDlgLindero() {
        try {
            lindero = new RegFichaLinderos();
            JsfUti.update("formFichaLindero");
            JsfUti.executeJS("PF('dlgFichaLindero').show()");
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
    }
    ...
    public void showDlgEditarLindero(RegFichaLinderos temporal) {
        try {
            lindero = temporal;
            JsfUti.update("formFichaLindero");
            JsfUti.executeJS("PF('dlgFichaLindero').show()");
    ...
    public void borrarLindero(int indice) {
        try {
            RegFichaLinderos temporal = linderantes.remove(indice);
            if (temporal.getId() != null) {
                manager.delete(temporal);
            }
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
    ...
    public void saveLinderos() {
        try {
            if (lindero.getTipo() == null) {
                JsfUti.messageError(null, "INGRESAR DATOS", "Debe ingresar el tipo de lindero.");
                return;
            }
            if (lindero.getLinderante() == null || lindero.getLinderante().isEmpty()) {
                JsfUti.messageError(null, "INGRESAR DATOS", "Debe ingresar el campo lindero.");
                return;
            }
            if (lindero.getId() == null) {
                linderantes.add(lindero);
            }
            JsfUti.update("mainForm:tabFicha:dtLinderos");
            JsfUti.executeJS("PF('dlgFichaLindero').hide();");
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
    }

    public void buscarAntecedentesFicha() {
        try {
            temps = new ArrayList<>();
            if (numeroFicha != null) {
                this.findFichaPrincipal();
                if (!this.movsficha.isEmpty()) {
                    JsfUti.update("frmMovs");
                    JsfUti.executeJS("PF('dlgMovimientos').show();");
                }
            } else {
                JsfUti.messageError(null, "INGRESAR DATOS", "Debe ingresar el número de ficha.");
            }
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
    ...
    public void agregarAntecedentesFicha() {
        try {
            if (temps.isEmpty()) {
                JsfUti.messageWarning(null, "SELECCIONAR MOVIMIENTO", "Debe seleccionar al menos un movimiento para agregar a la ficha.");
            } else {
                for (RegMovimiento temp : temps) {
                    RegMovimientoFicha mf = new RegMovimientoFicha();
                    mf.setMovimiento(temp);
                    movimientos.add(mf);
                    JsfUti.update("mainForm:tabFicha:dtMov");
    ...
    public void marcarControlCalidad() {
        try {
            if (this.validaCampos()) {
                ficha.setDireccionBien(ficha.getUafTipoBien().getDescripcion());
                if (ficha.getId() == null) {
                ...
                ficha.setUserControlCalidad(new AclUser(session.getUserId()));
                ficha.setFechaControlCalidad(new Date());
                ficha = reg.guardarFichaRegistral(ficha, propietarios, movimientos, linderantes);
                if (ficha == null) {
                    JsfUti.messageError(null, Messages.error, "");
                } else {
    ...
    public List<RegTipoLindero> getTiposLinderos() {
        return manager.findAllEntCopy(Querys.getRegTipoLinderos);
    }
    ...
    public RegFicha getFichaMain() {
        return fichaMain;
    }

    public void setFichaMain(RegFicha fichaMain) {
        this.fichaMain = fichaMain;
    }
    ...
    public List<RegFichaLinderos> getLinderantes() {
        return linderantes;
    }

    public void setLinderantes(List<RegFichaLinderos> linderantes) {
        this.linderantes = linderantes;
    }

    public RegFichaLinderos getLindero() {
        return lindero;
    }

    public void setLindero(RegFichaLinderos lindero) {
    ...
    public Boolean getAnalistaSenior() {
        return analistaSenior;
    }

    public void setAnalistaSenior(Boolean analistaSenior) {
        this.analistaSenior = analistaSenior;
    }

    public Boolean getJefeCertificacion() {
        return jefeCertificacion;
    }

    public void setJefeCertificacion(Boolean jefeCertificacion) {
        this.jefeCertificacion = jefeCertificacion;
    }
