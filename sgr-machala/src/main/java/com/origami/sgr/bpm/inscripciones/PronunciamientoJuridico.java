...
@Named
@ViewScoped
public class PronunciamientoJuridico extends BpmManageBeanBaseRoot implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(PronunciamientoJuridico.class.getName());
    ...
    protected Observaciones comentario;
    ...
    public void completarTarea() {
        try {
            if (this.validarTareasV3()) {
                this.guardarObservacion();
                this.reasignarTarea(this.getTaskId(), session.getName_user());
                par = new HashMap<>();
                par.put("aprobado", 1);
                par.put("jefeInscripcion", itl.getUsuarioByRolName("jefe_inscripcion"));
    ...
    public void completarTareaRechazo() {
        try {
            this.guardarObservacion();
            this.reasignarTarea(this.getTaskId(), session.getName_user());
            par = new HashMap<>();
    ...
    public void showDlgObservaciones() {
    ...
    public void completarTareaInscripciones() {
        try {
            par = new HashMap<>();
            if (this.validarTareasV1()) { //TODAS LAS TAREAS
                par.put("revision", 2);
    ...
    public void seleccionarCertificado() {
        try {
            ...
            ss.agregarParametro("taskID", this.getTaskId());
            session.setTaskID(this.getTaskId());
            JsfUti.redirectFaces("/procesos/inscripciones/certificarPI.xhtml");
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, null, e);
        }
    }
    ...
    public boolean validarTareasV1() {
        for (RegpTareasTramite tt : tareas) {
            if (!tt.getRealizado()) {
                return false;
            }
    ...
    public boolean validarTareasV2() {
    ...
    public boolean validarTareasV3() {
        for (RegpTareasTramite tt : tareas) {
            if (!tt.getRealizado() && !tt.getDetalle().getActo().getSolvencia()) {
                return false;
            }
    ...
    public void guardarNotaDevolutiva() {
        try {
            if (notaDevolutiva.getNumNotaDevolutiva() == null) {
                ...
                notaDevolutiva.setNumNotaDevolutiva(sec.getSecuenciaGeneralByAnio(Constantes.secuenciaNotaDevolutiva)
                        .toString() + "-" + anio);
            }
    ...
    public void showDlgEditarObservacion(Observaciones temp) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
            if (sdf.format(temp.getFecCre()).equals(sdf.format(new Date()))) {
                if (temp.getUserCre().equalsIgnoreCase(session.getName_user())) {
                    comentario = temp;
                    JsfUti.update("formEditarComentario");
                    JsfUti.executeJS("PF('dlgEditarComentario').show();");
                } else {
                    JsfUti.messageWarning(null, "No puede Editar", "Solo puede editar el comentario el mismo usuario que lo ingresó.");
                }
            } else {
                JsfUti.messageWarning(null, "No puede Editar", "Solo se puede editar el comentario durante el mismo día.");
            }
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, null, e);
        }
    ...
    public void editarObservacion() {
        try {
            if (comentario.getObservacion() == null || comentario.getObservacion().isEmpty()) {
                JsfUti.messageWarning(null, "Faltan Datos", "Debe ingresar un comentario.");
                return;
            }

            manager.update(comentario);
        } catch (Exception e) {
            JsfUti.messageError(null, Messages.error, "");
            LOGGER.log(Level.SEVERE, null, e);
        }
    ...
    public Observaciones getComentario() {
        return comentario;
    }

    public void setComentario(Observaciones comentario) {
