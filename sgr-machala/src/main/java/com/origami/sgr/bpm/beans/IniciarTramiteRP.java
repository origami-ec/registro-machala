        ...
        if (solicitante.getCiRuc() != null || solicitante.getCiRuc().isEmpty()) {
            if (!solicitante.getCiRuc().isEmpty()) {
                ...
                if (count == 1) {
                    ...
                    solicitante = this.validarClienteEgob(solicitante);
                } else {
                    solicitante = new CatEnte();
                }
        ...
        if (beneficiario.getCiRuc() != null || beneficiario.getCiRuc().isEmpty()) {
            if (!beneficiario.getCiRuc().isEmpty()) {
                ...
                if (count == 1) {
                    ...
                    beneficiario = this.validarClienteEgob(beneficiario);
                } else {
                    beneficiario = new CatEnte();
                }
...
//            }
    ...
    public void calculoContrato() {
        ...
        if (acto.getArancel().getCodigo() == 3) { //LEY ORGANICA DE MERCADO Y VALORES
    ...
    public boolean validaciones() {
        ...
        if (beneficiario.getId() == null) {
            JsfUti.messageWarning(null, "Debe ingresar Cliente para factura.", "");
            return false;
        }
        if (beneficiario.getIdEgob() == null) {
            JsfUti.messageWarning(null, "El Cliente para factura no se pudo registrar en el EGOB.", "");
            return false;
        }
 ...
 /*if (liquidacion.getEsJuridico()) {
    ...
    public void actualizarDatosBeneficiario() {
        try {
            this.validarClienteEgob(beneficiario);
    ...
    public void registrarEnEgob() {
        try {
            if (temporal.getGeneraFactura()) {
                if (itl.registrarLiquidacionEGOB(temporal.getId())) {
                    ...
                    block = true;
                    JsfUti.messageError(null, Messages.error, "NO se pudo registrar el Trámite Nro. "
                            + temporal.getNumTramiteRp() + " en el EGOB. Intente más tarde.");
                }
            } else {
                this.cargarDatosReporte();
                List<String> urlList = new ArrayList<>();
