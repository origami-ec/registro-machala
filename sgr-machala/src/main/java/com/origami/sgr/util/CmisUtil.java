...
public class CmisUtil implements Serializable {
    ...
    public ContentStream getDocumentSession(String id) {
        Session se = this.sesionAlfresco;
        try {
            SessionFactory sessionFactory = SessionFactoryImpl.newInstance();
            Map<String, String> parameters = new HashMap<>();
            parameters.put(SessionParameter.USER, "admin");
            parameters.put(SessionParameter.PASSWORD, "322pitR223");//322pitR223
            parameters.put(SessionParameter.ATOMPUB_URL, "http://localhost:8282/alfresco/service/cmis");
            parameters.put(SessionParameter.BINDING_TYPE, BindingType.ATOMPUB.value());
            se = sessionFactory.getRepositories(parameters).get(0).createSession();
            Document dc = (Document) se.getObject(id);
            return dc.getContentStream();
        } catch (Exception e) {
            Logger.getLogger(CmisUtil.class.getName()).log(Level.SEVERE, null, e);
            return null;
