...
@Named
@ViewScoped
public class PcEmitirCertificados extends BpmManageBeanBaseRoot implements Serializable {
    ...
    protected Boolean certificador, tieneNotaDevolutiva = false;
    ...
    protected void iniView() {
        try {
            //System.out.println("// firma: " + session.getFirma());
    ...
    public void completarTarea() {
        try {
            if (this.validarFirmaElectronica()) {
                if (this.validarTareas()) {
                    this.guardarObservacion();
                    this.reasignarTarea(this.getTaskId(), session.getName_user());
                    par = new HashMap<>();
                    par.put("aprobado", 1);
                    par.put("jefeCertificacion", itl.getUsuarioByRolName("jefe_certificacion"));
    ...
    public void completarTareaFirma() {
        try {
            if (this.validarTareas()) {
                as.generarFirmaEnviarCorreoCertificadoIndividual(liquidacion, session.getName_user());
            ...
            JsfUti.messageError(null, "Error al completar Tarea.", "");
        }
    }
    ...
    public void imprimirNotaDevolutiva(RegpNotaDevolutiva dev) {
        try {
            ...
            ss.setNombreReporte("NotaDevolutivaCertificado");
    ...
    public void finalizarTarea(Long numTramite) {
        ...
        String obs = "Se envio el documento al correo: "
                + (!liq.getSolicitante().getCorreo1().isEmpty() ? liq.getSolicitante().getCorreo1() : "")
                + " - " + liq.getSolicitante().getNombreCompleto();
        ...
        if (!taskId.isEmpty()) {
            ...
            if (liq.getCertificadoSinFlujo()) {
                reg.guardarObservaciones(ht1, Constantes.tramiteDescargado, obs,
                        Constantes.certificadoExpress);
            }
        }
    }
    ...
    public void rechazar() {
        try {
            ...
            /*notaDevolutiva.setFirma(user.getEnte().getNombresApellidos()
                    + Variables.saltoLinea + Variables.tituloCertificador);*/
        } catch (Exception e) {
            System.out.println(e);
        }
    }
    ...
    public void seleccionarCertificado() {
        try {
            ...
            ss.agregarParametro("taskID", this.getTaskId());
            ss.agregarParametro("formulario", "/procesos/certificacion/emitirCertificado.xhtml");
            session.setTaskID(this.getTaskId());
            JsfUti.redirectFaces("/procesos/registro/certificar.xhtml");
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
    ...
    public void completarTareaDevolutiva() {
        try {
            ...
            as.generarFirmaEnviarCorreoDevolutivaCertificado(notaDevolutiva, liquidacion, session.getName_user());
            observacion = "NOTA DEVOLUTIVA: " + notaDevolutiva.getAsunto();
            this.guardarObservacion();
            this.reasignarTarea(this.getTaskId(), session.getName_user());
    ...
    public void solicitarConsultaSenior() {
        try {
            ...
            par = new HashMap<>();
            par.put("aprobado", 3);
            par.put("analistaSenior", itl.getUsuarioByRolName("analista_senior_certificacion"));
